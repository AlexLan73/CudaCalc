/**
 * @file fft32_optimized_v2.cu
 * @brief FFT32 with memory optimizations
 * 
 * Optimizations:
 * - __ldg() for read-only cache
 * - Padding +2 for bank conflict avoidance
 * - Const correctness
 */
...
__global__ void fft32_optimized_v2_kernel(
    ...
    const int tid = threadIdx.y;  // 0-31: which point (renamed from y to tid for clarity!)
    const int fft_id = blockIdx.x * 32 + x;
    
    if (fft_id >= num_windows) return;
    
    // Shared memory: [32 FFT][32 points] + padding
    __shared__ float2 data[32][34];
    
    // Load with bit-reversal + __ldg() optimization
    const int input_idx = fft_id * 32 + tid;
    const int reversed_tid = bitReverse5(tid);
    const cuComplex val = __ldg(&input[input_idx]);  // Read-only cache!
    data[x][reversed_tid] = make_float2(val.x, val.y);
    __syncthreads();
...
extern "C" void launch_fft32_optimized_v2(
    const cuComplex* d_input,
    cuComplex* d_output,
    int num_windows
) {
    dim3 block(32, 32);  // [32 FFT, 32 points]
    int num_blocks = (num_windows + 31) / 32;
    dim3 grid(num_blocks);
    
    fft32_optimized_v2_kernel<<<grid, block>>>(d_input, d_output, num_windows);
}
