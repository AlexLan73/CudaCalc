/**
 * @file fft32_batch32.cu
 * @brief FFT32 with 32 butterflies per block
 * 
 * Block: [32, 32] = 1024 threads
 * 32 FFT32 processed in parallel per block
 */
...
__global__ void fft32_batch32_kernel(
    ...
    if (y < 16) {
        ...
        const int twiddle_idx = (j * 32) / 2;  // j * 16
    ...
    if (y < 16) {
        ...
        const int twiddle_idx = (j * 32) / 4;  // j * 8
    ...
    if (y < 16) {
        ...
        const int twiddle_idx = (j * 32) / 8;  // j * 4
        const float tw_cos = TWIDDLE_32_COS[twiddle_idx];
        const float tw_sin = TWIDDLE_32_SIN[twiddle_idx];
    ...
    if (y < 16) {
        ...
        const int twiddle_idx = (j * 32) / 16;  // j * 2
        const float tw_cos = TWIDDLE_32_COS[twiddle_idx];
        const float tw_sin = TWIDDLE_32_SIN[twiddle_idx];
    ...
    if (y < 16) {
        ...
        const int twiddle_idx = j;  // (j * 32) / 32 = j
        const float tw_cos = TWIDDLE_32_COS[twiddle_idx];
        const float tw_sin = TWIDDLE_32_SIN[twiddle_idx];
...
// Host launcher
extern "C" void launch_fft32_batch32(
    const cuComplex* d_input,
    cuComplex* d_output,
    int num_windows
) {
    dim3 block(32, 32);  // [32 FFT, 32 points] = 1024 threads
    int num_blocks = (num_windows + 31) / 32;
    dim3 grid(num_blocks);
    
    fft32_batch32_kernel<<<grid, block>>>(d_input, d_output, num_windows);
}
