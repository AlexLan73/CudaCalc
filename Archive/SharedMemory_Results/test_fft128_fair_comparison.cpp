...
extern "C" void launch_fft128_shared_memory(const cuComplex*, cuComplex*, int);
extern "C" void launch_fft128_simple_single(const cuComplex*, cuComplex*, int);

int main() {
    std::cout << "\n=== FFT128 FAIR COMPARISON: Shared Memory vs Simple Single ===\n\n";
    
    const int num_windows = 1024;
    const int points_per_window = 128;
    const int total_points = num_windows * points_per_window;
    
    // Generate test signal: exp(2πi*n/128) for each window
    ...
    std::cout << "Testing FFT128 Shared Memory...\n";
    launch_fft128_shared_memory(d_input, d_output_shared, num_windows);
    cudaDeviceSynchronize();
    
    cudaError_t cuda_err = cudaGetLastError();
    if (cuda_err != cudaSuccess) {
        std::cout << "FFT128 Shared Memory kernel failed: " << cudaGetErrorString(cuda_err) << "\n";
        return 1;
    }
    
    // Test simple single version
    std::cout << "Testing FFT128 Simple Single...\n";
    launch_fft128_simple_single(d_input, d_output_simple, num_windows);
    cudaDeviceSynchronize();
    
    cuda_err = cudaGetLastError();
    if (cuda_err != cudaSuccess) {
        std::cout << "FFT128 Simple Single kernel failed: " << cudaGetErrorString(cuda_err) << "\n";
        return 1;
    }
