/**
 * @file fft64_shared_memory_v2.cu
 * @brief FFT64 GPU kernel using shared memory - 1 FFT per block
 * 
 * Исправленная версия: 1 FFT64 на блок вместо 16
 * Shared memory: 64 точки × 8 байт = 512 байт
 */
...
__constant__ float cos_table_64_v2[32];
__constant__ float sin_table_64_v2[32];

// Initialize twiddle factors for FFT64
void init_fft64_twiddles_v2() {
    ...
    cudaMemcpyToSymbol(cos_table_64_v2, h_cos, 32 * sizeof(float));
    cudaMemcpyToSymbol(sin_table_64_v2, h_sin, 32 * sizeof(float));
}

// 6-bit bit reversal
__device__ __forceinline__ int bitReverse6_v2(int x) {
    int result = 0;
    #pragma unroll
    for (int i = 0; i < 6; ++i) {
        result = (result << 1) | (x & 1);
...
__global__ void fft64_shared_memory_v2_kernel(const cuComplex* input, cuComplex* output, int num_windows) {
    ...
    // Stage 1: m=2, m2=1, distance=1, 32 butterflies
    {
        ...
        if (idx_in_group < m2) {  // only first half of each group does work
            ...
            const float tw_real = cos_table_64_v2[tw_idx];
            const float tw_imag = sin_table_64_v2[tw_idx];
...
// Host launcher
extern "C" void launch_fft64_shared_memory_v2(const cuComplex* input, cuComplex* output, int num_windows) {
    // Initialize twiddle factors
    init_fft64_twiddles_v2();
    
    dim3 block(64);  // 64 threads per block, 1 FFT per block
    dim3 grid(num_windows);  // One block per FFT
    
    fft64_shared_memory_v2_kernel<<<grid, block>>>(input, output, num_windows);
}
