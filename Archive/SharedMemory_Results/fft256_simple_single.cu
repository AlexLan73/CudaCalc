/**
 * @file fft256_simple_single.cu
 * @brief FFT256 Simple version - 1 FFT per block
 * 
 * Честное сравнение с Shared Memory версией
 * 1 FFT256 на блок, 256 потоков на блок
 */
...
__constant__ float cos_table_256_single[128];
__constant__ float sin_table_256_single[128];

// Initialize twiddle factors for FFT256
void init_fft256_twiddles_single() {
    ...
    cudaMemcpyToSymbol(cos_table_256_single, h_cos, 128 * sizeof(float));
    cudaMemcpyToSymbol(sin_table_256_single, h_sin, 128 * sizeof(float));
}

// 8-bit bit reversal
__device__ __forceinline__ int bitReverse8_single(int x) {
    int result = 0;
    #pragma unroll
    for (int i = 0; i < 8; ++i) {
        result = (result << 1) | (x & 1);
...
__global__ void fft256_simple_single_kernel(const cuComplex* input, cuComplex* output, int num_windows) {
    ...
    // Shared memory: 256 points (same as Shared Memory version)
    __shared__ cuComplex data[256];
    
    // Load input data with bit reversal
    const int input_idx = fft_id * 256 + tid;
    const int bit_rev_idx = bitReverse8_single(tid);
    data[bit_rev_idx] = input[input_idx];
    
    __syncthreads();
    
    // FFT256: 8 stages (точно такой же алгоритм как в Shared Memory версии)
    // Stage 1: m=2, m2=1, distance=1, 128 butterflies
    {
        ...
        if (idx_in_group < m2) {
            ...
            const float tw_real = cos_table_256_single[tw_idx];
            const float tw_imag = sin_table_256_single[tw_idx];
...
// Host launcher
extern "C" void launch_fft256_simple_single(const cuComplex* input, cuComplex* output, int num_windows) {
    // Initialize twiddle factors
    init_fft256_twiddles_single();
    
    dim3 block(256);  // 256 threads per block, 1 FFT per block
    dim3 grid(num_windows);  // One block per FFT
    
    fft256_simple_single_kernel<<<grid, block>>>(input, output, num_windows);
}
