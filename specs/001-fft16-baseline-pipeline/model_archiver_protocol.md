...
- Сравниваем по метрике **Compute ms** при одинаковых: размере N, количестве окон (batch), числе потоков и объёмах загрузки/выгрузки. Для прикладной оценки throughput учитываем и **Total ms**.
