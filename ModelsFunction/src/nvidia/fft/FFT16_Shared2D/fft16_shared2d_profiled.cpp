/**
 * @file fft16_shared2d_profiled.cpp
 * @brief FFT16_Shared2D with profiling implementation
 */
...
OutputSpectralData FFT16_Shared2D_Profiled::process_with_profiling(
    const InputSignalData& input,
    BasicProfilingResult& profiling
) {
    if (!initialized_) {
        throw std::runtime_error("FFT16_Shared2D_Profiled not initialized!");
    }
